...
export default function A1Z26Tool({ toolMeta: _ }: ToolComponentProps) {
  ...
  const run = (mode: "encode" | "decode") => {
    const r = process({ input, mode, delimiter });
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      <div className="flex items-center gap-2">
        ...
        <Input value={delimiter} onChange={(e) => setDelimiter(e.target.value)} className="font-mono max-w-[80px]" />
        <span className="text-xs text-muted-foreground">between numbers (e.g. - or ,)</span>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <Button size="sm" onClick={() => run("encode")} disabled={!input.trim()}>Encode</Button>
        <Button size="sm" variant="outline" onClick={() => run("decode")} disabled={!input.trim()}>Decode</Button>
      ...
      <SplitPane
        left={<CodeEditor value={input} onChange={setInput} language="plaintext" placeholder="Enter text or A1Z26 numbers..." />}
        right={

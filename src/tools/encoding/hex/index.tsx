...
export default function HexTool({ toolMeta: _ }: ToolComponentProps) {
  const [input, setInput] = useState("");
  const [output, setOutput] = useState("");
  const [outputKey, setOutputKey] = useState(0);
  const [error, setError] = useState("");
  const [delimiter, setDelimiter] = useState<"space" | "none" | "0x" | "comma">("space");
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      <div className="flex flex-wrap items-center gap-2">
        <Button size="sm" onClick={() => run("encode")} disabled={!input.trim()}>To Hex</Button>
        <Button size="sm" variant="outline" onClick={() => run("decode")} disabled={!input.trim()}>From Hex</Button>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Select value={delimiter} onValueChange={(v) => setDelimiter(v as typeof delimiter)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="0x">0x prefix</SelectItem>
      ...
      <SplitPane
        left={<CodeEditor value={input} onChange={setInput} language="plaintext" placeholder="Enter text or hex..." />}
        right={

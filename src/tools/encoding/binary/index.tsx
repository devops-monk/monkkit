...
export default function BinaryTool({ toolMeta: _ }: ToolComponentProps) {
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      <div className="flex flex-wrap items-center gap-2">
        <Button size="sm" onClick={() => run("encode")} disabled={!input.trim()}>To Binary</Button>
        <Button size="sm" variant="outline" onClick={() => run("decode")} disabled={!input.trim()}>From Binary</Button>
      ...
      <SplitPane
        left={<CodeEditor value={input} onChange={setInput} language="plaintext" placeholder="Enter text or binary string..." />}
        right={
